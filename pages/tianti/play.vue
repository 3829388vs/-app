<template>
	...
	<view class="container">
		...
		<view class="winlogo" v-if="showlose2">
			<image class="winlogoImg" style="width: 100%;height: 650rpx" src="../../static/imgs/lose.gif"></image>
		</view>
		<image class="mygjAnm" :src="mygjAnm" v-if="showMygjAnm"></image>
		<equipment v-if="openEq" :lists="myEquip" :tools="myTool" :moneys="money" @setMoney="setMoney" @showEq="openEquip" @eqlist="getEqlist" @toolList="getToolList"></equipment>
		<view class="main">
...
export default {
	data() {
		return {
			...
			flag: 0,
			flag2: 0,
			bgImg: '../../static/imgs/hero/diyuBg.jpg',
			bgImg2: '../../static/imgs/hero/diyuBg.jpg',
			heroTX: 'herogj.gif',
			enemyTX: 'herogj.gif',
			showTX: false,
			showTX2: false,
	...
	methods: {
		...
		enemyHarm() {
			...
			if(this.enemy.name == '人间-花木兰'){
				if(this.randomNum(0,2) == 1 && this.bgImg.indexOf('fb_hml') == -1){
					this.bgImg = '../../static/imgs/hero/fb_hml2.gif?'+ new Date().getTime()
				...
				if(this.randomNum(0,3) == 0 && this.bgImg.indexOf('fb_hml') == -1){
					uni.showToast({
						title: '兄长，看招！',
						icon:'none',
						duration: 1000
			...
			if(this.enemy.name == '传说-金武圣人'){
				...
				if(this.randomNum(0,3) == 1 && this.bgImg.indexOf('fb_lq') == -1){
					...
					let val = this.enemy.gj*2.3 - this.myHero.fy*1.5 + this.randomNum(200,700)
					this.shotDown = true
					setTimeout(()=>{
						this.shotDown = false
					},1500)
			...
			if(this.enemy.name == '人间-项羽'){
				if(this.randomNum(0,2) == 1 && this.bgImg.indexOf('fb_xy') == -1){
					this.bgImg = '../../static/imgs/hero/fb_xy.gif?'+ new Date().getTime()
			...
			if(this.enemy.name == '人间-刘邦'){
				if(this.randomNum(0,2) == 1){
					...
					this.bgImg = '../../static/imgs/hero/fb_lb.gif?'+ new Date().getTime()
			...
			if(this.enemy.name == '人间-赵云'){
				if(this.randomNum(0,1) == 1 && this.bgImg.indexOf('fb_zzl') == -1){
					uni.showToast({
						title: '在下赵子龙也',
						icon:'none',
						duration: 1500
					});
					this.bgImg = '../../static/imgs/hero/fb_zzl.gif?'+ new Date().getTime()
			...
			if(this.enemy.name == '人间-关羽'){
				if(this.randomNum(0,3) == 1 && this.bgImg.indexOf('fb_gy') == -1){
					uni.showToast({
						title: '来者何人，报上名来',
						icon:'none',
						duration: 1500
					});
					this.bgImg = '../../static/imgs/hero/fb_gy.gif?'+ new Date().getTime()
				...
				if(this.bloodVal1 <= 30 && this.bloodVal1 > 2 && this.bgImg.indexOf('fb_gy') == -1){
					uni.showToast({
						title: '我和你拼了！！',
						icon:'none',
						duration: 1500
			...
			if(this.enemy.name == '成龙历险记-圣主'){
				if(this.randomNum(0,3) == 1 && this.bgImg.indexOf('fb_sz') == -1){
					...
					let val = this.enemy.gj*2.6 - this.myHero.fy*1.2 + this.randomNum(100,900)
					this.shotDown = true
					setTimeout(()=>{
						this.shotDown = false
					},2300)
			...
			if(this.enemy.name == '杨贵妃'){
				if(this.randomNum(0,2) == 1 && this.bgImg.indexOf('fb_ygf') == -1){
					this.bgImg = '../../static/imgs/hero/fb_ygf.gif?'+ new Date().getTime()
					this.backBgImg('fb_ygf.gif',2200)
					uni.showToast({
						title: '许久不见，君可好？',
						icon:'none',
						duration: 1500
					...
					setTimeout(()=>{
						this.blood1 = parseInt(this.blood1) + 500
						this.bloodVal1 = Math.round(parseInt(this.blood1) / parseInt(this.enemy.blood) * 100)
						if(this.enemy.blood<this.blood1){
							this.enemy.blood = this.blood1
						}
						this.enemyBeat2(this.randomNum(parseInt(this.enemy.gj),parseInt(this.enemy.gj * 2)))
					},1800)
				}
				if(this.randomNum(0,2) == 0 && this.bgImg.indexOf('fb_yyh') == -1){
					uni.showToast({
						title: '汝且听我演奏一曲',
						icon:'none',
						duration: 1000
			...
			if((this.myHero.id == 5 || this.myHero.name == '鲲') && this.bloodVal2 <= 33 && this.flag2 == 0){
				if(this.myHero.name == '鲲'){
					...
					if(this.myHero.id == 5 && parseInt(this.addData.strongIndex) == 1){
						this.addBlood(parseInt(350 + this.myHero.fs * 1.5),0,0)
					}
		...
		heroHarm () {
			if(this.myEquipName.includes('兰德里的折磨') && !this.beatTime){
				this.consistBeat(30+this.myHero.fs*0.4,700,2800)
			}
		...
		beat(beatVal,type) {
			...
			if(parseInt(this.myHero.fs) >= 260){
				fsval = fsval * 0.8
			}
			if(gjval <= 0){
				gjval = 0
			...
			if(parseInt(this.shotVal2) < 50){
				this.shotVal2 = 30 + this.randomNum(0,20)
			}
		...
		regGj() {
			...
			let val = (parseInt(this.myHero.gj * 2.5) - parseInt(this.enemy.fy * 1.8)) * 1.5 + this.regGjAdd.gjHarm
			let hasBj = false
			//暴击
			if(this.randomNum(0,10) <= (parseFloat(this.myHero.bj) * 10)){
				...
				if(val <= 0){
					...
					if(this.myEquipName.includes('无尽之刃')) {
						val = (val - this.regGjAdd.gjHarm) * 2.5 + this.regGjAdd.gjHarm	//暴击需减去gjHarm伤害
					}else{
		...
		getLoopEnmList(num) {
			...
			if(num == 204){
				...
				this.bgImg2 = '../../static/imgs/hero/bg1.jpeg'
			}
			if(num == 205){
			...
			if(num == 208){
				...
				this.bgImg = '../../static/imgs/hero/yaoBg.jpg'
				this.bgImg2 = '../../static/imgs/hero/yaoBg.jpg'
			}
			if(num == 209){
				this.money = editMoney(300)
			...
			if(num == 211){
				...
				this.bgImg = '../../static/imgs/hero/tianting.jpg'
				this.bgImg2 = '../../static/imgs/hero/tianting.jpg'
			}
			if(num == 212){
