				...
				<view class="roles">
					<view class="item" v-for="item in heroList" :key="item.id">
						<view class="left" @click="sel(item)">
							...
							<view :class="{'heroName':true,'deadText':item.isLive == 0}">{{ item.name }}</view>
						</view>
	...
	methods: {
		...
		sel(item) {
			...
			if(uni.getStorageSync('gameType') == 'paiwei'){
				uni.redirectTo({
					url: "/pages/paiwei/index?item="+JSON.stringify(item)+'&num='+this.num
				})
				return
			}
			uni.redirectTo({
				url: "/pages/tianti/index?item="+JSON.stringify(item)+'&num='+this.num
			})
			// if(item.name == '德玛西亚之力'){
			// 	this.playVoice('https://mp-d8b0f60b-69cf-48cd-be25-d721256c4d17.cdn.bspapp.com/game/dmxyzl.mp3')
			// }
			// if(item.name == '少林武僧'){
			// 	this.playVoice('https://mp-d8b0f60b-69cf-48cd-be25-d721256c4d17.cdn.bspapp.com/game/wqds.mp3')
			// }
			// if(item.name == '无极剑圣'){
			// 	this.playVoice('https://mp-d8b0f60b-69cf-48cd-be25-d721256c4d17.cdn.bspapp.com/game/wjjs.mp3')
			// }
			// if(item.name == '广寒仙子-嫦娥'){
			// 	this.playVoice('https://mp-d8b0f60b-69cf-48cd-be25-d721256c4d17.cdn.bspapp.com/game/cer.mp3')
			// }
			// if(this.type == 'fuben') {
			// 	uni.redirectTo({
