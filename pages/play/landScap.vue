				...
				<view class="my">
					...
					<view class="tallent">
						<view class="tallentRow">
							<tallent ref="tallents" :noflz="flz == 0" :name="myHero.name" :heroId="myHero.id" :begin="begin"
...
export default {
	data() {
		return {
			...
			riverTime2: null,
			tempEnemyIcon: null,
			myImmunity: false,	//我方免疫异常状态
			enImmunity: false,	//敌方免疫异常状态
			enFlag: false,	//敌方flag
			enFlagTime: null,	//敌方时间flag
			iconStyle: '',	//头像装饰
			flyStyle: ''	//羽翼
		}
	},
	...
	mounted() {
		...
		if(this.gameType.indexOf('tanxian') == -1){
			...
			if(this.gameType.indexOf('tanxian') == -1){
				if(this.gameType == 'paiwei'){
					if(this.reallyRandom(0,1) == 1){
						self.playVoice('../../static/sounds/battleBg.wav',true,10,180000)
					}else{
						self.playVoice('../../static/sounds/battleBg2.wav',true,10,180000)
					}
				}else{
					self.playVoice('../../static/sounds/bgMp3.wav',true,10,170000)
				}
			}
			//头像相框
	...
	methods: {
		beginBettle() {
			...
			if(JSON.parse(uni.getStorageSync('myStore')).length != 0){
				JSON.parse(uni.getStorageSync('myStore')).forEach(i=>{
					if(i.isOther && i.name.indexOf('羽翼') != -1 && this.second == 0 && this.min == 0){
						if(i.fy != 0){
							this.myHero.fy = parseInt(this.myHero.fy) + i.fy
						}
						if(i.speed && this.myspeed - parseFloat(i.speed) >= 0.4){
							this.myspeed -= parseFloat(i.speed).toFixed(2)
						}
					}
				})
			}
		...
		enemyBeat() {
			...
			this.time1 = setInterval(()=>{
				...
				//排位赛敌方英雄伤害、技能
				if((this.gameType == 'paiwei' || this.gameType == 'match' || this.gameType == 'shijiebei' || this.gameType.indexOf('tanxian') != -1) && this.enemy.isHero){
					if(this.randomNum(0,1) == 1){
						...
						if(this.enemyBeatCD == null){
							...
							if(this.distanceComp(this.enemy.dis2)){
								setTimeout(()=>{
									this.enemyTallent(2)
								},1000 + this.randomNum(0,2)*500)
							}
							if(this.distanceComp(this.enemy.dis3)){
								setTimeout(()=>{
									this.enemyTallent(3)
								},2500 + this.randomNum(0,2)*500)
							}
							if(this.distanceComp(this.enemy.dis4)){
								setTimeout(()=>{
									this.enemyTallent(4)
								},3300 + this.randomNum(0,2)*800)
							}
							this.enemyBeatCD = 1
						}
					}
		...
		enemyBeat2(val) {
				...
				this.shotVal1 = (val - parseInt(this.myHero.fy * this.fyS)) * 1.5
		...
		enemyHarm() {
			...
			if(this.myEquipName.includes('石像石板甲')) {
				if(this.randomNum(0,1) == 1 && this.distanceComp(this.enemy.distance)){
					this.enemy.gj = this.enemy.gj - 2
				}
			}
			...
			if(this.enemy.name == '南宫问天' && this.distanceComp(800)){
				if(this.randomNum(0,2) == 1 && this.engjAnm.indexOf('fb_ngwt') == -1){
					...
					this.playVoice('神兵天晶！',true,5,2000,'aiying')
					uni.showToast({
						title: '神兵天晶！',
						icon:'none',
						duration: 1200
			...
			if(this.enemy.name == '哆啦A梦-小夫' && this.distanceComp(130)){
				if(this.randomNum(0,2) == 1){
					...
					this.playVoice('小子！',true,5,2000,'zhifeng_emo')
					this.enChongci(130)
			...
			if(this.enemy.name == '神奇宝贝-杰尼龟' && this.distanceComp(800)){
				if(this.randomNum(0,2) == 1){
					...
					this.playVoice('俺可不是吃素的！',true,4,2000,'aiying')
			...
			if(this.enemy.name == '羊村-灰太狼' && this.distanceComp(800)){
				if(this.bloodVal1 <= 16 && this.bloodVal1 > 2){
					...
					this.playVoice('我又回来了！',true,4,2000,'zhibing_emo')
					this.enDuobi(50)
			...
			if(this.enemy.name == '猪猪侠-超人强' && this.distanceComp(150)){
				if(this.randomNum(0,2) == 1){
					...
					this.playVoice('小样！也敢和我决斗！',true,5,2000,'zhibing_emo')
			...
			if(this.enemy.name == '传说-金武圣人'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_lq') == -1 && this.distanceComp(150)){
					...
					this.playVoice('施主看招！',true,5,2000,'zhifeng_emo')
					this.showEngjAnm = true
				...
				if(this.randomNum(0,4) == 0 && this.distanceComp(100)){
					...
					this.playVoice('放下屠刀吧！',true,5,2000,'zhifeng_emo')
			...
			if(this.enemy.name == '杨贵妃'){
				...
				if(this.randomNum(0,3) == 0 && this.engjAnm.indexOf('fb_yyh') == -1 && this.distanceComp(160)){
					...
					this.playVoice('汝且听我演奏一曲！',true,4,2000,'zhimiao_emo')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '武侠-扫地僧' || this.enemy.name == '扫地僧'){
				...
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_lq') == -1 && this.distanceComp(130)){
					...
					this.playVoice('施主看招！',true,4,2000,'zhifeng_emo')
					this.enChongci(130)
					this.tallentExpert(1,2,3000)
					this.showEngjAnm = true
					this.engjAnm = '../../static/imgs/hero/fb_lq.gif?'+ new Date().getTime()
			...
			if(this.enemy.name == '加勒比海盗王'){
				...
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_plk') == -1 && this.distanceComp(130)){
					...
					this.playVoice('是谁动了我蛋糕？！',true,5,2000,'zhifeng_emo')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '海贼王-路飞'){
				...
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_hzw') == -1){
					...
					this.playVoice('觉醒！',true,4,2000,'zhifeng_emo')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '齐天大圣'){
				if(this.randomNum(0,1) == 1 && this.bloodVal1 <= 33 && this.bloodVal1 > 2){
					...
					this.playVoice('../../static/sounds/cdbyjl.mp3',true,5,2500)
				}
				if(this.randomNum(0,2) == 1 && this.engjAnm.indexOf('fb_swk') == -1 && this.distanceComp(120)){
					this.playVoice('../../static/sounds/qtds3.wav',true,6,2000)
					setTimeout(()=>{
						uni.showToast({
							title: '妖怪哪里跑！',
							icon:'none',
						...
						this.playVoice('../../static/sounds/ygnlp.mp3',false,7,3500)
					},2200)
					this.showEngjAnm = true
					...
					setTimeout(()=>{
						...
						this.playVoice('../../static/sounds/calsyb.mp3',false,4,2500)
			...
			if(this.enemy.name == '葫芦兄弟'){
				if(this.randomNum(0,1) == 1 && this.engjAnm.indexOf('fb_hlw') == -1 && this.bloodVal1 <= 50 && this.bloodVal1 > 2){
					...
					this.playVoice('兄弟连心！',true,4,2000,'aiying')
					this.showEngjAnm = true
				...
				if(this.randomNum(0,2) == 1 && this.engjAnm.indexOf('fb_hlw') == -1 && this.distanceComp(90)){
					...
					this.playVoice('看我排山蹈海！',true,5,2000,'aiying')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '花木兰'){
				if(this.randomNum(0,2) == 1 && this.engjAnm.indexOf('fb_hml') == -1 && this.distanceComp(120)){
					...
					this.playVoice('兄长，吃我一剑！',true,4,2000,'aicheng')
					this.enChongci(120)
				...
				if(this.randomNum(0,2) == 0 && this.engjAnm.indexOf('fb_hml') == -1 && this.distanceComp(65)){
					...
					this.playVoice('兄长，看招！',true,5,2000,'aicheng')
					this.showEngjAnm = true
					this.engjAnm = '../../static/imgs/hero/fb_hml.gif?'+ new Date().getTime()
					this.backBgImg('fb_hml',4200)
			...
			if(this.enemy.name == '血耻鳄鱼'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_ey') == -1 && this.distanceComp(100)){
					...
					this.playVoice('来吧！伙计',true,5,2000,'laotie')
					this.showEngjAnm = true
					this.enChongci(100)
			...
			if(this.enemy.name == '哪吒'){
				if(this.randomNum(0,3) == 0 && this.engjAnm.indexOf('fb_nz') == -1){
					...
					this.playVoice('我命！！由我不由天！',true,4,2000,'aiying')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '玄武'){
				if(this.randomNum(0,3) == 0 && this.engjAnm.indexOf('fb_xw') == -1){
					...
					this.playVoice('世界唯我独尊！',true,4,2000,'zhifeng_emo')
					this.showEngjAnm = true
			...
			if(this.enemy.name.indexOf('托塔天王') != -1){
				if(this.randomNum(0,2) == 1){
					this.fasheArrow(150,1,75,'敌方射击',2,18,4)
				}
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('tttw2') == -1){
					this.heroAnm('../../static/imgs/hero/tttw2.gif?'+ new Date().getTime(),1)
					this.backBgImg('tttw2',2500,1)
					if(this.time4){
						let val = this.enemy.gj*2.5 - this.myHero.fy*1.5
						this.enemyBeat2(val)
					}else{
						this.myHero.v = parseFloat(this.myHero.v + 0.3).toFixed(2)
						this.tallentExpert(1,2,1700)
						this.time4 = setInterval(()=>{
							if(this.enemyX > this.characterX){
								this.characterX = this.characterX + 7
							}else{
								this.characterX = this.characterX - 7
							}
							if(this.enemyY > this.characterY){
								this.characterY = this.characterY + 7
							}else{
								this.characterY = this.characterY - 7
							}
						},100)
				...
				if(this.randomNum(0,3) == 1){
					this.enChongci(50,1)
					if(this.blood1 + 500 <= this.enemy.blood){
						this.blood1 = parseInt(this.blood1) + 150 + parseInt(this.enemy.gj*1.3)
						this.bloodVal1 = Math.round(parseInt(this.blood1) / parseInt(this.enemy.blood) * 100)
					}
				}
				if(this.randomNum(0,4) == 3 && this.enmapObjects.length == 0){
						this.tallentExpert(3,2,1000)
						this.enmapObjects.push({
							left: this.characterX - 50,
							top: this.characterY - 180,
							width: 330,
							height: 450,
							img: 'baota.png'
						});
						this.time4 = setInterval(()=>{
							this.enmapObjects.forEach((i,d)=>{
								if(i.img == 'baota.png'){
									i.top += 7
								}
							})
						},120)
						let rand = this.randomNum(0,1000)
						setTimeout(()=>{
							clearInterval(this.time4)
							this.time4 = true
							this.delayTel(4000-rand,0)
							this.tallentExpert(1,2,4000-rand)
							let val = this.enemy.gj*3.5 - this.myHero.fy*1.5 + this.randomNum(100,500)
							this.enemyBeat2(val)
						},1200)
						setTimeout(()=>{
							this.time4 = null
			...
			if(this.enemy.name == '成龙历险记-圣主'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_sz') == -1 && this.distanceComp(800)){
					...
					this.playVoice('恶龙激光波',true,4,2000,'zhifeng_emo')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '秦始皇'){
				if(this.randomNum(0,3) == 1 && this.distanceComp(200)){
					uni.showToast({
						title: '反贼，拿命来！',
						icon:'none',
						duration: 1000
					});
					this.playVoice('反贼，拿命来！',true,4,2000,'zhifeng_emo')
					this.enChongci(250)
					this.enemyBeat2(parseInt(this.enemy.gj)*2 - parseInt(this.myHero.fy)+this.randomNum(200,600))
				}
				if(this.bloodVal1 <= 35 && this.bloodVal1 > 2 && this.randomNum(0,2) == 1 && this.enemy.fy < 300){
					uni.showToast({
						title: '众军护驾！',
						icon:'none',
						duration: 1000
					});
					this.playVoice('众军护驾！',true,5,2000,'zhifeng_emo')
					this.enemy.fy += 80
					setTimeout(()=>{
						this.enemy.fy -= 80
					},3500)
			...
			if(this.enemy.name == '妖族-奎木狼'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_kml') == -1 && this.distanceComp(130)){
					...
					this.playVoice('看招！',true,5,2000,'zhifeng_emo')
					this.enChongci(130)
					//禁锢
			...
			if(this.enemy.name == '妖族-石矶娘娘'){
				if(this.randomNum(0,2) == 1 && this.distanceComp(100)){
					...
					this.playVoice('给我灰飞烟灭！',true,5,2000,'xiaoxian')
				...
				if(this.bloodVal1 <= 23 && this.bloodVal1 > 2){
					if(this.randomNum(0,1) == 1){
						...
						this.playVoice('嘿！挺能打的嘛！',true,5,2000,'xiaoxian')
					}
				}
			}
			if(this.enemy.name == '妖族-北海龙王'){
				if(this.bloodVal1 <= 50 && this.bloodVal1 > 2){
					if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_bhlw') == -1){
						...
						this.playVoice('眼前是哪位勇士？',true,5,2000,'laotie')
						this.showEngjAnm = true
			...
			if(this.enemy.name == '妖族-西海龙王' || this.enemy.name == '西海龙王-敖闰'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_lkz') == -1){
					...
					this.playVoice('裂空爪！',true,5,2000,'siqi')
					this.showEngjAnm = true
				...
				if(this.bloodVal1 <= 35 && this.bloodVal1 > 2 && this.randomNum(0,2) == 2 && this.engjAnm.indexOf('fb_lkz') == -1){
					...
					this.playVoice('走了！再见！',true,5,2000,'siqi')
					this.yinshen2 = true
					setTimeout(()=>{
						this.yinshen2 = false
			...
			if(this.enemy.name == '神-哪吒'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_nz2') == -1){
					...
					this.playVoice('哪吒在此！',true,5,2000,'aiying')
					this.showEngjAnm = true
					this.playVoice('../../static/sounds/jiguangbo.wav',false,6,3000)
				...
				if(this.randomNum(0,3) == 0 && this.engjAnm.indexOf('fb_nz') == -1){
					...
					this.playVoice('我命！由我不由天！',true,5,2000,'aiying')
					this.showEngjAnm = true
					this.engjAnm = '../../static/imgs/hero/fb_nz.gif?'+ new Date().getTime()
					this.backBgImg('fb_nz',6900)
					if(this.enspeed >= 0.7){
			...
			if(this.enemy.name == '神-韩湘子'){
				if(this.randomNum(0,3) == 1 && this.enspeed != 0.2 && this.engjAnm.indexOf('fb_hxz') == -1){
					...
					this.playVoice('八仙过海！',true,5,2000,'xiaogang')
			...
			if(this.enemy.name == '神-十万天兵'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_swtb') == -1 && this.distanceComp(80)){
					...
					this.playVoice('杀！杀！杀！',true,5,2000,'laotie')
				...
				if(this.randomNum(0,4) == 1 && this.engjAnm.indexOf('fb_swtb') == -1){
					...
					this.showEngjAnm = true
					this.enDuobi(60)
					this.playVoice('集合！',true,5,2000,'laotie')
			...
			if(this.enemy.name == '神-四大天王'){
				if(this.randomNum(0,3) == 0 && this.engjAnm.indexOf('fb_sdtw') == -1 && this.distanceComp(110)){
					...
					this.playVoice('持国天王：破！',true,5,2000,'laotie')
					this.showEngjAnm = true
				...
				if(this.randomNum(0,4) == 1 && this.engjAnm.indexOf('fb_sdtw') == -1){
					...
					this.playVoice('多闻天王：转！',true,6,2000,'laotie')
					this.showEngjAnm = true
				...
				if(this.randomNum(0,4) == 1 && this.engjAnm.indexOf('fb_sdtw') == -1 && this.enspeed != 0.5){
					...
					this.playVoice('增长天王：进！',true,4,2000,'laotie')
					this.showEngjAnm = true
				...
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_sdtw') == -1 && this.distanceComp(500)){
					...
					this.playVoice('广目天王：电！',true,4,2000,'laotie')
					this.showEngjAnm = true
			...
			if(this.enemy.name == '神-二郎神杨戬'){
				if(this.randomNum(0,3) == 1 && this.engjAnm.indexOf('fb_yj') == -1 && this.distanceComp(100)){
					...
					this.playVoice('杨戬在此！谁敢造次',true,4,2000,'laotie')
			...
			if(this.enemy.name == '神-齐天大圣'){
				...
				if(this.randomNum(0,2) == 1 && this.engjAnm.indexOf('fb_swk') == -1 && this.distanceComp(120)){
					...
					this.playVoice('../../static/sounds/ygnlp.mp3',false,6,3500)
					this.showEngjAnm = true
					this.engjAnm = '../../static/imgs/hero/fb_swk.gif?'+ new Date().getTime()
					...
					setTimeout(()=>{
						...
						this.playVoice('../../static/sounds/calsyb.mp3',false,7,2500)
						let val = this.enemy.gj*2.3 - this.myHero.fy*1.5 + this.randomNum(200,600)
						setTimeout(()=>{
							this.enemyBeat2(val)
							this.shotDown = true
			...
			this.computeDistance()
			//百花仙子
			if(this.enemy.id == 1002 && this.enImmunity && this.distance <= 150){
				let val = parseInt(this.enemy.gj*1.5)
				this.enemyBeat2(val)
			}
			if(this.myHero.id == 1002 && this.myImmunity && this.distance <= 150){
				let val = parseInt(this.myHero.fs*1.2) + parseInt(this.addData.add11)*50
				this.beat(val,1)
			}
		},
		//已方发动技能触发事件
		heroHarm () {
			...
			if(this.myEquipName.includes('星界驱驰') && !this.beatTime){
				this.heroAddSpeed(0.3,2000)
			}
			if(this.myEquipName.includes('风暴狂涌') && !this.beatTime){
				setTimeout(()=>{
					this.beat(this.reallyRandom(1,3)*100+50,1)
				},2000)
			}
		...
		tallentHandle(data) {
			...
			if(this.myHero.name == '德玛西亚之力' || this.myHero.id == 1){
				...
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhifeng_emo')
				if(num == 1){
			...
			if(this.myHero.name == 'GG Bond' || this.myHero.id == 3) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'aiying')
				if(num == 1){
			...
			if(this.myHero.name == '少林武僧' || this.myHero.id == 2) {
				...
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'laotie')
				if(num == 1){
					if(strongIndex == 1){
			...
			if(this.myHero.name == '无极剑圣' || this.myHero.id == 4){
				...
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhibing_emo')
				if(num == 1){
			...
			if(this.myHero.name == '广寒仙子-嫦娥' || this.myHero.id == 5){
				...
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'xiaoxian')
				if(num == 1){
			...
			if(this.myHero.name == '鲲' || this.myHero.id == 6){
				...
				if(num == 1){
					...
					setTimeout(()=>{
						...
						this.enemy.gj = parseInt(this.enemy.gj*0.6)
						setTimeout(()=>{
							this.enemy.gj = parseInt(this.enemy.gj/0.6)
							this.enemy.gj = this.enemy.gj - 3 - addTalent
						},2000 + addTalent*250)
					},900)
				}
				if(num == 2){
			...
			if(this.myHero.name == '精英战士' || this.myHero.id == 7){
				...
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'xiaogang')
				if(num == 1){
					//狙击枪
					if(this.ygbj){
			...
			if(this.myHero.name == '风神' || this.myHero.id == 8){
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'maoxiaomei')
				if(num != 1){
			...
			if(this.myHero.name == '诺克萨斯之手' || this.myHero.id == 9){
				...
				if(num == 4){
					...
					if(parseInt(this.blood1)/parseInt(this.enemy.blood) <= 0.35){
						cval = parseInt(this.myHero.gj*0.7) + 250 + addTalent4*50
					}
					let val = parseInt(this.myHero.gj * 1.35) + 350 + cval + parseInt(this.xuli)*170 + addTalent4*120
					setTimeout(()=>{
						this.beat(val)
						this.xuli = 0
					},1300)
			...
			if(this.myHero.name == '探险家' || this.myHero.id == 10){
				...
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'aicheng')
				if(num == 1){
			...
			if(this.myHero.name == '齐天大圣' || this.myHero.id == 101){
				...
				if(num == 3){
					...
					if(this.reallyRandom(0,1) == 0){
						this.playVoice('../../static/sounds/ygnlp.mp3',false,6,3600)
					}else{
						this.playVoice('../../static/sounds/alsly.mp3',false,6,3600)
					}
					this.delayTel(1000,num)
			...
			if(this.myHero.name == '玉玲珑' || this.myHero.id == 11){
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'aiting')
				this.xuli += 1
				if(this.xuli >= 3){
			...
			if(this.myHero.name == '达摩' || this.myHero.id == 18){
				...
				if(num == 4){
					...
					setTimeout(()=>{
						...
						if(this.enemyX <= this.characterX){
							if(strongIndex == 1){
								this.enemyX = this.enemyX - 130 - addTalent4*10
							}else{
			...
			if(this.myHero.name == '阿宁' || this.myHero.id == 23){
				this.playVoice('../../static/sounds/aning'+num+'.wav',true,num-1,1200)
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'aiting')
				if(num == 1){
					this.cd(5000 - addTalent*250,1)
					this.blueComput(30 + addTalent*3,'reduce')
					if(strongIndex == 1){
						this.fasheArrow(170+ addTalent*5,2,30,this.myHero.id,1,9,10)
					}else{
						if(strongIndex == 2){
							this.fasheArrow(160+ addTalent*5,2,30,this.myHero.id,1,9,8)
						}else{
							this.fasheArrow(135+ addTalent*5,2,30,this.myHero.id,1,9,8)
						}
					}
				}
				if(num == 2){
					this.cd(9000 - addTalent2*250,2)
					this.blueComput(80 + addTalent2*10,'reduce')
					this.delayTel(2500,num)
					this.heroAnm('../../static/imgs/hero/aning2.gif?'+ new Date().getTime())
					this.backBgImg('aning2',2200)
					this.shotDown = true
					setTimeout(()=>{
						this.shotDown = false
					},2500)
					let val = parseInt(this.myHero.gj*1.8) - parseInt(this.enemy.fy*1.5) + 50 + this.randomNum(20,100) + addTalent2*35
					if(val<50){
						val = 50
					}
					this.consistBeat(val,500,2500)
					if(this.reallyRandom(0,2) == 2){
						//被动
						this.fasheArrow(120,2,30,this.myHero.id,1,9,7)
					}
				}
				if(num == 3){
					this.cd(7500 - addTalent3*250,3)
				...
				if(num == 4){
					...
					this.delayTel(1500,num)
					this.blueComput(100 + addTalent4*10,'reduce')
					if(this.bloodVal2 <= (20 + addTalent4*3)){	//生命值小于20%~30%
						this.addBlood(333+ addTalent4*90 + parseInt(parseInt(this.myHero.fy)/1.5),500,1500)
						this.tallentExpert(3,2,1500)
					}else{
						this.addBlood(170+ addTalent4*22 + parseInt(parseInt(this.myHero.fy)/3),500,1500)
					}
					if(strongIndex == 2){
						this.shotDown = true
						setTimeout(()=>{
							this.shotDown = false
						},3000 + this.reallyRandom(0,3)*700)
						let val = parseInt(this.myHero.gj*1.7) - parseInt(this.enemy.fy*1.5) + 50 + this.randomNum(20,50)
						if(val<50){
							val = 50
						}
						this.consistBeat(val,500,3000 + this.reallyRandom(0,3)*700)
					}
				}
			}
			if(this.myHero.name == '亡灵战神' || this.myHero.id == 24){
				this.playVoice('../../static/sounds/aning'+num+'.wav',true,num-1,1200)
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhifeng_emo')
				if(num == 1){
					if(this.xuli != 0){
						this.cd(8500 - addTalent*250,1)
						this.blueComput(50 + addTalent*5,'reduce')
						this.heroAnm('../../static/imgs/hero/wlzs1.gif?'+ new Date().getTime())
						this.backBgImg('wlzs1',2000)
						if(this.distanceComp(110 + addTalent*3)){
							this.tallentExpert(2,1,1000 + this.xuli*120)
							let val = this.harmComputer(1,addTalent) + 200 + parseInt(this.xuli)*30 + this.randomNum(0,this.xuli*22) + addTalent*50
							this.beat(val)
						}
						this.xuli = 0
						clearInterval(this.ylzz)
					}else{
						this.delayTel(2500,1)
						this.ylzz = setInterval(()=>{
							this.xuli += 1
						},200)
						setTimeout(()=>{
							this.$refs.tallents.tallentShow = true
				...
				if(num == 2){
					this.cd(8000 - addTalent2*250,2)
					this.blueComput(50 + addTalent2*5,'reduce')
					this.myHero.fy = this.myHero.fy + 40 + addTalent2*5
					this.addBlood(200 + parseInt(this.myHero.fy*2),0,0)
					setTimeout(()=>{
						if(this.distanceComp(138)){
							let val = this.myHero.fs + 100 + this.randomNum(0,200) + addTalent2*30
							this.beat(val,1)
						}
						this.myHero.fy = this.myHero.fy - 40 - addTalent2*5
					},3500)
				...
				if(num == 3){
					this.cd(5000 - addTalent3*250,3)
					this.blueComput(30 + addTalent3*3,'reduce')
					this.fasheArrow(160,2,36,this.myHero.id,3,2,2)
				}
				if(num == 4){
					this.cd(13000 - addTalent4*250,4)
					this.blueComput(120 + addTalent4*10,'reduce')
					...
					this.myNoMove = true
					this.showTX(2,2)
					this.fastsp = this.myspeed
					if(this.characterX >= this.enemyX){
						this.ygbj = setInterval(()=>{
							this.computeDistance()
							if(this.distanceComp(40)){
								this.tallentExpert(2,1,2500)
								clearInterval(this.ygbj)
								let val = this.harmComputer(1,addTalent4) + 500  + this.randomNum(0,300) + addTalent4*70
								this.beat(val)
								this.$refs.tallents.tallentShow = true
								this.$refs.tallents.tallentShow2 = true
								this.$refs.tallents.tallentShow3 = true
								...
								if(this.characterX < 20){
									...
									this.characterX = this.characterX - 12
								}
							}
						},50)
					}else{
						this.ygbj = setInterval(()=>{
							...
							if(this.distanceComp(40)){
								this.tallentExpert(2,1,2500)
								clearInterval(this.ygbj)
								let val = this.harmComputer(1,addTalent4) + parseInt(this.myHero.gj*1.5) + 500 - parseInt(this.enemy.fy*1.5) + this.randomNum(0,300) + addTalent4*70
								this.beat(val)
								this.$refs.tallents.tallentShow = true
								this.$refs.tallents.tallentShow2 = true
								this.$refs.tallents.tallentShow3 = true
								...
								if(this.characterX < this.screenWidth * 1.3){
									this.characterX = this.characterX + 12
								}else{
									this.$refs.tallents.tallentShow = true
									this.$refs.tallents.tallentShow2 = true
			...
			if(this.myHero.name == '韦鲁斯' || this.myHero.id == 1000){
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'xiaogang')
				if(this.fastsp < 0.4){
					this.fastsp = parseFloat(this.fastsp + 0.05).toFixed(3)
				}
				if(num == 1){
					this.cd(6500 - addTalent*250,1)
					this.blueComput(50 + addTalent*5,'reduce')
					this.delayTel(1000,0)
					this.heroAnm('../../static/imgs/hero/wls1.gif?'+ new Date().getTime())
					this.backBgImg('wls1',1800)
					this.fasheArrow(520,2,32,this.myHero.id,1,9,3)
				}
				if(num == 2){
					this.cd(5000 - addTalent2*250,2)
					this.blueComput(30 + addTalent2*5,'reduce')
					this.fasheArrow(400,2,38,this.myHero.id,2,8,3)
				}
				if(num == 3){
					this.cd(7500 - addTalent3*250,3)
					this.blueComput(30 + addTalent3*3,'reduce')
				...
				if(num == 4){
					...
					this.delayTel(1500,num)
					this.blueComput(100 + addTalent4*10,'reduce')
					this.fasheArrow(600,2,36,this.myHero.id,4,5,3)
					if(this.enemyX < this.characterX){
						this.characterX = this.characterX + (80 + this.randomNum(0,50))
					}else{
						this.characterX = this.characterX - (80 + this.randomNum(0,50))
					}
				}
			}
			if(this.myHero.name == '狂暴奎木狼' || this.myHero.id == 1001){
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'laotie')
				if(num == 1){
					this.cd(5500 - addTalent*250,1)
					this.blueComput(35 + addTalent*5,'reduce')
					this.delayTel(500,0)
					this.heroAnm('../../static/imgs/hero/jfly1.gif?'+ new Date().getTime())
					this.backBgImg('jfly1',1000)
					let val = this.harmComputer(1,addTalent) + 250 + addTalent*30
				...
				if(num == 2){
					this.playVoice('../../static/sounds/kbkml.mp3',true,num-1,3000)
					this.cd(11000 - addTalent2*350,2)
					this.blueComput(30,'reduce')
					this.delayTel(1000,0)
					setTimeout(()=>{
						this.tallentExpert(5,1)
					},500)
					if(parseFloat(this.myHero.v)>0.6){
						this.myHero.v = parseFloat(this.myHero.v - 0.2 - addTalent2*0.05).toFixed(2)
						setTimeout(()=>{
							this.myHero.v = parseFloat(parseFloat(this.myHero.v) + 0.2 + addTalent2*0.05).toFixed(2)
						},3500+addTalent2*500)
					}
				}
				if(num == 3){
					this.playVoice('../../static/sounds/chongji.wav',true,num-1,1800)
					this.cd(7500 - addTalent3*250,3)
					this.blueComput(50 + addTalent3*5,'reduce')
					this.heroChongci(130)
					//禁锢
					this.tallentExpert(1,1,1500)
					this.heroAnm('../../static/imgs/hero/kbkml3.gif?'+ new Date().getTime())
					this.backBgImg('kbkml3',900)
					this.beat(this.harmComputer(1,addTalent3) + 150)
				}
				if(num == 4){
					this.playVoice('../../static/sounds/kbkml4.mp3',true,num-1,4000)
					this.cd(16000 - addTalent4*250,4)
					this.blueComput(100+addTalent4*10,'reduce')
					this.delayTel(500,0)
					setTimeout(()=>{
						this.tallentExpert(6,2,3800 + addTalent4*1000)
					},500)
				}
			}
			if(this.myHero.name == '百花仙子' || this.myHero.id == 1002){
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhimiao_emo')
				if(num == 1){
					this.cd(7500 - addTalent*250,1)
					this.playVoice('../../static/sounds/bhxz1.wav',true,num-1,2000)
					this.blueComput(60 + addTalent*6,'reduce')
					this.fasheArrow(450,2,35,"百花仙子",1,2,2)
				}
				if(num == 2){
					this.playVoice('../../static/sounds/bhxz2.wav',true,num-1,3000)
					this.cd(13000 - addTalent2*350,2)
					this.blueComput(100,'reduce')
					this.delayTel(2000,0)
					let blval = parseInt((600 + addTalent2*150 + this.myHero.fs*2)/2)
					this.addBlood(blval,1000,2050)
				}
				if(num == 3){
					this.cd(8000 - addTalent3*250,3)
					this.blueComput(70 + addTalent3*5,'reduce')
					this.fasheArrow(350,2,30,"百花仙子",2,16,1)
				}
				if(num == 4){
					this.playVoice('../../static/sounds/bhxz4.wav',true,num-1,4000)
					this.cd(13000 - addTalent4*250,4)
					this.blueComput(100+addTalent4*5,'reduce')
					this.delayTel(500,0)
					this.myImmunity = true
					setTimeout(()=>{
						this.myImmunity = false
					},5000+addTalent4*1000)
				}
			}
			if(this.myHero.name == '牛魔王' || this.myHero.id == 1003){
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'laotie')
				if(num == 1){
					this.cd(6000 - addTalent*200,1)
					this.blueComput(50 + addTalent*5,'reduce')
					this.delayTel(600,0)
					setTimeout(()=>{
						let val = this.harmComputer(2,addTalent) + 150
						this.beat(val)
						this.tallentExpert(2,1,1000+addTalent*100)
					},600)
				}
				if(num == 2){
					this.cd(9500 - addTalent2*250,2)
					this.blueComput(100,'reduce')
					let fyval = parseInt(this.myHero.fy*(0.3 + addTalent2*0.02))
					let blood2 = this.blood2
					this.myHero.fy = this.myHero.fy + fyval
					setTimeout(()=>{
						this.myHero.fy = this.myHero.fy - fyval
						if(this.blood2 < blood2 && this.distanceComp(135)){
							let val = parseInt((blood2 - this.blood2)*0.5) + 250 - this.enemy.mk*2
							this.beat(val)
						}
					},3500 + addTalent2*500)
				}
				if(num == 3){
					this.cd(10000 - addTalent3*250,3)
					this.blueComput(50 + addTalent3*5,'reduce')
					this.heroChongci(120)
					let val = this.harmComputer(2,addTalent3)
					this.beat(val)
					setTimeout(()=>{
						this.enDuobi(70+ addTalent3*10)
						this.tallentExpert(2,1,1000+addTalent3*100)
					},100)
				}
				if(num == 4){
					this.cd(15000 - addTalent4*250,4)
					this.blueComput(100+addTalent4*5,'reduce')
					this.delayTel(1500,0)
					setTimeout(()=>{
						let val = this.harmComputer(2,addTalent4) + 500 + this.randomNum(0,300)
						this.beat(val)
						this.pojia = this.pojia - 0.25 - addTalent4*0.03
						this.pomo = this.pomo - 0.25 - addTalent4*0.03
						this.enemy.fy = this.enemy.fy - 6 - addTalent4
						this.enemy.mk = this.enemy.mk - 6 - addTalent4
						setTimeout(()=>{
							this.pojia = this.pojia + 0.25 + addTalent4*0.03
							this.pomo = this.pomo + 0.25 + addTalent4*0.03
						})
			...
			if(this.myHero.name == '狂暴武僧' || this.myHero.id == 1004) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'laotie')
				let randomVal = this.randomNum(50,250)
				if(num == 1){
					this.cd(3800 - addTalent*200,1)
				...
				if(num == 3){
					...
					this.blueComput(100 + addTalent3*15,'reduce')
					let val = Math.round(this.shotVal1) + Math.round(120 / parseFloat(this.enemy.gs)) + addTalent3*150
					+ parseInt(this.myHero.fs) + parseInt(this.myHero.gj*1.3) - parseInt(this.enemy.mk*0.6*this.pomo) - parseInt(this.enemy.fy*(0.6*this.pojia)) + randomVal
					if(val<=0){
						val = randomVal
					}
					setTimeout(()=>{
				...
				if(num == 4){
					...
					this.backBgImg('wqds',1500)
					this.myHero.fy = parseInt(this.myHero.fy) + 30 + addTalent4*10
					this.myHero.gj = parseInt(this.myHero.gj) + 20 + addTalent4*5
					this.myHero.fs = parseInt(this.myHero.fs) + 30 + addTalent4*5
					setTimeout(()=>{
						this.myHero.fy = parseInt(this.myHero.fy) - 30 - addTalent4*10
						this.myHero.gj = parseInt(this.myHero.gj) - 20 - addTalent4*5
						this.myHero.fs = parseInt(this.myHero.fs) - 30 - addTalent4*5
			...
			if(this.myHero.name == '雷伊' || this.myHero.id == 1005) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'aicheng')
				if(this.blood1 <= 50 && this.xuli == 0){
				...
				if(num == 1){
					this.cd(5000 - addTalent*250,1)
					this.blueComput(35 + addTalent*3,'reduce')
					this.fasheArrow(450,2,35,this.myHero.id,1,16,2)
				}
				if(num == 2){
					this.cd(10000 - addTalent2*250,2)
					this.blueComput(60 + addTalent2*10,'reduce')
					this.playVoice('../../static/sounds/ly2.mp3',true,num-1,4000)
					this.heroAnm('../../static/imgs/hero/ly2.gif?'+ new Date().getTime())
					this.backBgImg('ly2',1200)
					let fsv = parseInt(this.myHero.fs*(0.25 + addTalent2*0.02))
					this.myHero.fs = this.myHero.fs + fsv
					this.myHero.gj = this.myHero.gj + 50 + addTalent2*10
					this.fastsp = 0.15
					setTimeout(()=>{
						this.myHero.fs = this.myHero.fs - fsv
						this.myHero.gj = this.myHero.gj - 50 - addTalent2*10
				...
				if(num == 3){
					this.cd(7000 - addTalent3*250,3)
					this.blueComput(60 + addTalent3*5,'reduce')
					this.playVoice('../../static/sounds/ly3.wav',true,num-1,2000)
					this.heroChongci(120)
					if(this.characterX > this.enemyX){
					...
					if(this.characterY > this.enemyY){
						this.characterY = this.enemyY - 40
					}else{
						this.characterY = this.enemyY + 40
					}
					let val = this.harmComputer(1,addTalent3) + 250 + addTalent3*50
					this.beat(val)
					if(!this.shotDown){
						this.shotDown = true
						setTimeout(()=>{
				...
				if(num == 4){
					this.cd(11000 - addTalent4*350,4)
					this.blueComput(130 + addTalent4*10,'reduce')
					this.heroChongci(138)
					this.playVoice('../../static/sounds/ly4.mp3',true,num-1,3000)
					this.heroAnm('../../static/imgs/hero/ly4.gif?'+ new Date().getTime())
					this.backBgImg('ly4',1000)
					setTimeout(()=>{
						let val = this.harmComputer(2,addTalent4) + parseInt(this.myHero.fs) + 350 + addTalent4*70
						this.beat(val)
						this.tallentExpert(4,1,5000)
					},60)
				}
			}
			if(this.myHero.name == '地狱火' || this.myHero.id == 1006) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhibing_emo')
				if(num == 1){
					this.cd(5000 - addTalent*250,1)
					this.blueComput(35 + addTalent*3,'reduce')
					this.fasheArrow(400,2,35,this.myHero.id,1,1,1)
				}
				if(num == 3){
					this.cd(8000 - addTalent3*250,3)
					this.blueComput(70 + addTalent3*7,'reduce')
					this.playVoice('../../static/sounds/ly3.wav',true,num-1,2000)
					this.delayTel(500,0)
					this.xuli = 1
					setTimeout(()=>{
				...
				if(num == 4){
					this.cd(13500 - addTalent4*350,4)
					this.blueComput(150 + addTalent4*15,'reduce')
					this.fasheArrow(400,2,38,this.myHero.id,4,3,2)
				}
			}
			if(this.myHero.name == '二郎真君-杨戬' || this.myHero.id == 1007) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhibing_emo')
				if(num == 1){
					if(this.ylzz){
						return
					}
					this.playVoice('../../static/sounds/elzj1.mp3',true,num-1,2500)
					this.blueComput(50 + addTalent*5,'reduce')
					this.tallentObjects.push({
						left: this.characterX - this.randomNum(30,70),
						top: this.characterY + this.randomNum(20,60),
						width: 130,
						height: 136,
						img: 'xtq.png'
					});
					...
					this.ylzz = setInterval(()=>{
						if(this.enemyX > this.tallentObjects[ind].left){
							this.tallentObjects[ind].left = this.tallentObjects[ind].left + 3
						}else if(this.tallentObjects[ind].left - this.enemyX > 36){
							this.tallentObjects[ind].left = this.tallentObjects[ind].left - 3
						}
						if(this.enemyY > this.tallentObjects[ind].top){
							this.tallentObjects[ind].top = this.tallentObjects[ind].top + 3
						}else if(this.tallentObjects[ind].top - this.enemyY > 10){
							this.tallentObjects[ind].top = this.tallentObjects[ind].top - 3
						}
						//哮天犬已靠近
						if(Math.abs(this.tallentObjects[ind].left - this.enemyX) <= (60 + addTalent*5) && Math.abs(this.tallentObjects[ind].top - this.enemyY) <= 50 && this.xuli == 0){
							setTimeout(()=>{
								clearInterval(this.ylzz)
								this.ylzz = null
								this.xuli = 0
								this.tallentObjects.splice(ind,1)
								this.cd(6000 - addTalent*250,1)
							},2000)
							let val = parseInt(this.myHero.gj*1.7) - parseInt(this.enemy.fy*1.5) + 100 + this.randomNum(20,50)
							if(val<50){
								val = 50
							}
							this.consistBeat(val,500,2000)
							this.xuli = 1
						}
					},30)
				}
				if(num == 2){
					this.cd(8000 - addTalent2*250,2)
					this.blueComput(30 + addTalent2*3,'reduce')
					this.regGjAdd.gjHarm = parseInt(this.regGjAdd.gjHarm) + parseInt(this.myHero.gj*(0.15 + addTalent2*0.05))
					if(this.myspeed > 0.3){
						this.myspeed = this.myspeed - 0.3 - addTalent2*0.02
					}
					setTimeout(()=>{
						this.regGjAdd.gjHarm = parseInt(this.regGjAdd.gjHarm) - parseInt(this.myHero.gj*(0.15 + addTalent2*0.05))
						this.myspeed = this.myspeed + 0.3 + addTalent2*0.02
				...
				if(num == 3){
					...
					this.blueComput(25 + addTalent3*2,'reduce')
					this.playVoice('../../static/sounds/elzj3.wav',true,num-1,2500)
					this.fasheArrow(350,2,45,this.myHero.id,3,6,5)
					this.heroAnm('../../static/imgs/hero/elzj3.gif?'+ new Date().getTime())
					this.backBgImg('elzj3',2000)
				}
				if(num == 4){
					this.cd(12000 - addTalent4*300,4)
					this.blueComput(100 + addTalent4*10,'reduce')
					this.playVoice('../../static/sounds/chongji.wav',true,num-1,1800)
					this.heroChongci(200)
					let val = this.harmComputer(1,addTalent4) + 500 + addTalent4*100
					//敌方生命值小于30%
					if(parseInt(this.blood1)/parseInt(this.enemy.blood) <= 0.3){
						val = parseInt(val*1.5)
					}
					this.beat(val)
					this.heroAnm('../../static/imgs/hero/elzj4.gif?'+ new Date().getTime())
					this.backBgImg('elzj4',1500)
				}
			}
			if(this.myHero.name == '无量仙翁' || this.myHero.id == 1008) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'laotie')
				this.regGjAdd.gjHarm = parseInt(this.myHero.fs*1.5)
				if(num == 1){
					this.cd(7000 - addTalent*250,1)
					this.blueComput(60 + addTalent*6,'reduce')
					this.fasheArrow(450,2,35,this.myHero.id,1,16,2)
				}
				if(num == 2){
					...
					this.blueComput(50 + addTalent2*5,'reduce')
					//生命值小于30%
					if(parseInt(this.blood2)/parseInt(this.myHero.blood) <= 0.3){
						this.myHero.fy = this.myHero.fy + 70 + addTalent2*15
					}else{
						this.myHero.fy = this.myHero.fy + parseInt((70 + addTalent2*15)*1.5)
					}
					setTimeout(()=>{
						if(parseInt(this.blood1)/parseInt(this.enemy.blood) <= 0.3){
							this.myHero.fy = this.myHero.fy - 70 - addTalent2*15
				...
				if(num == 3){
					this.cd(8500 - addTalent3*500,3)
					this.blueComput(80 + addTalent3*7,'reduce')
					this.delayTel(500,0)
					setTimeout(()=>{
						if(this.reallyRandom(0,1) == 0){
							this.shotDown = true
							...
							this.enspeed = this.enspeed + 0.3 + addTalent3*0.05
							setTimeout(()=>{
								this.enspeed = this.enspeed - 0.3 - addTalent3*0.05
							},3000+addTalent3*500)
							let val = this.harmComputer(2,addTalent3) + 250 + this.randomNum(0,150)
							this.beat(val)
						}else{
							this.tallentExpert(9,1,3000+addTalent3*500)
							let val = this.harmComputer(2,addTalent3) + 250 + this.randomNum(0,150)
							this.beat(val)
						}
					},1500)
				...
				if(num == 4){
					this.cd(15000 - addTalent4*1000,4)
					this.blueComput(150 + addTalent4*15,'reduce')
					this.tallentObjects.push({
						left: this.enemyX - this.randomNum(30,70),
						top: this.enemyY - 280,
						width: 390,
						height: 390,
						img: 'tianyuanding.png'
					});
					let ind = 0
					this.tallentObjects.map((i,d)=>{
					  if(i.img == 'tianyuanding.png'){
						  ind = d
					  }
					})
					this.ylzz = setInterval(()=>{
						this.tallentObjects[ind].top = this.tallentObjects[ind].top + 20
						//装住
						if(Math.abs(this.tallentObjects[ind].left - this.enemyX) <= 150 && this.enemyY - this.tallentObjects[ind].top <= 150){
							let val = parseInt(this.myHero.fs*1.6) - parseInt(this.enemy.mk*1.5) + 100 + this.randomNum(0,50)
							if(val<50){
								val = 50
							}
							this.consistBeat(val,500,3000)
			...
			if(this.myHero.name == '不朽战神' || this.myHero.id == 1009) {
				this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'zhifeng_emo')
				if(num == 1){
					this.cd(5000 - addTalent*250,1)
					this.delayTel(300,0)
					this.blueComput(50 + addTalent*5,'reduce')
					this.playVoice('../../static/sounds/bxzs1.wav',true,num-1,1500)
					let val = this.harmComputer(1,addTalent) + 250
					this.beat(val)
					this.tallentExpert(3,1,3000,0.35+addTalent*0.05)
				}
				if(num == 2){
					this.cd(11000 - addTalent2*250,2)
					this.playVoice('../../static/sounds/bxzs2.wav',true,num-1,1500)
					this.blueComput(90 + addTalent2*5,'reduce')
					this.myHero.icon = ''
					//沉默
					this.shotDown = true
					this.yinshen = true
					...
					setTimeout(()=>{
						this.computeDistance()
						if(this.distanceComp(250)){
							this.shotDown = false
							this.yinshen = false
							this.enemyMoveContrl(2)
							this.myHero.icon = this.origHero.icon
							this.heroChongci(250)
							this.playVoice('../../static/sounds/chongji.wav',true,num-1,1800)
							if(this.characterX > this.enemyX){
								this.characterX = this.enemyX - 30
							}else{
								this.characterX = this.enemyX + 30
							}
							if(this.characterY > this.enemyY){
							...
							let val = this.harmComputer(1,addTalent2) - 150
							this.beat(val)
							this.tallentExpert(5,1)
						}else{
							setTimeout(()=>{
				...
				if(num == 3){
					this.cd(8000 - addTalent3*300,3)
					this.blueComput(30 + addTalent3*7,'reduce')
					this.heroChongci(135)
					this.playVoice('../../static/sounds/chongji.wav',true,num-1,1800)
					let val = this.harmComputer(1,addTalent3) + parseInt(this.myHero.gj*1.5)
					this.beat(val)
				}
				if(num == 4){
					this.cd(14500 - addTalent4*500,4)
					this.delayTel(1000,0)
					this.playVoice('../../static/sounds/bxzs4.mp3',true,num-1,1800)
					this.blueComput(100 + addTalent4*10,'reduce')
					setTimeout(()=>{
						this.fasheArrow(500,2,50,this.myHero.id,4,18)
					},1000)
				}
			}
			if(this.myHero.name == '狂暴孙悟空' || this.myHero.id == 1010) {
				this.playVoice('../../static/sounds/qtds'+num+'.wav',true,num-1,4000)
				if(num == 1){
					this.delayTel(2500,num)
					this.playVoice('../../static/sounds/calsyb.mp3',false,7,2500)
					...
					this.enemy.fy = parseInt(this.enemy.fy*(0.7-addTalent*0.05))
					setTimeout(()=>{
						this.enemy.fy = parseInt(this.enemy.fy/(0.7-addTalent*0.05))
					},3500 + addTalent*200)
					this.enemy.fy = this.enemy.fy - 5 - addTalent
					this.enemy.mk = this.enemy.mk - 5 - addTalent
				}
				if(num == 2){
					this.cd(9500 - addTalent2*150,2)
					this.delayTel(300,num)
				...
				if(num == 3){
					this.cd(6800 - addTalent3*300,3)
					if(this.reallyRandom(0,1) == 0){
						this.playVoice('../../static/sounds/ygnlp.mp3',false,6,3600)
					}else{
						this.playVoice('../../static/sounds/alsly.mp3',false,6,3600)
					}
					this.delayTel(700,num)
					this.heroAnm('../../static/imgs/hero/swk3.gif?'+ new Date().getTime())
					this.backBgImg('swk3',4000)
					this.blueComput(80 + addTalent3*10,'reduce')
					if(parseFloat(this.myHero.v)>=0.75){
						this.myHero.v = parseFloat(this.myHero.v - 0.3 - addTalent3*0.05).toFixed(2)
						setTimeout(()=>{
							this.myHero.v = parseFloat(parseFloat(this.myHero.v) + 0.3 + addTalent3*0.05).toFixed(2)
						},3000)
					}else{
						let v = this.myHero.v
						this.myHero.v = 0.4
					...
					let val = this.harmComputer(1,addTalent3) + 300 + addTalent3*60
					if(this.yinshen){
						val += parseInt(this.myHero.gj*1.5)
						this.shotDown = false
					}
					this.beat(val)
				}
				if(num == 4){
					this.cd(12000 - addTalent4*200,4)
					this.blueComput(120 + addTalent4*15,'reduce')
					this.delayTel(4000,num)
					this.heroAnm('../../static/imgs/hero/swk4.gif?'+ new Date().getTime())
					this.backBgImg('swk4',4000)
					this.follow(1,4000)
					this.myNoMove = true
					this.cd5 = false
					this.myHero.fy = parseInt(this.myHero.fy + this.myHero.fy*0.5)
					...
					let val = parseInt(this.myHero.gj * (1.6+addTalent4*0.1)) - parseInt(this.enemy.fy*1.7*this.pojia) + parseInt(this.myHero.fs*0.5)
					  + addTalent4*15 + this.randomNum(20,70)
					  if(val < 30){
						  val = 30
					  }
			...
			if(this.myHero.name == '女警' || this.myHero.id == 103){
				if(num != 4){
					this.playVoice(this.myHero.tallent[num].name,true,num-1,2000,'aiting')
				}
				if(num == 1){
					this.cd(3500 - addTalent*200,1)
					this.blueComput(20 + addTalent*2,'reduce')
					this.fasheArrow(350,2,25,this.myHero.id,1,12,3)
					this.playVoice('../../static/sounds/jyzs1-1.wav',true,num-1,3000)
				}
				if(num == 3){
					if(strongIndex == 1){
						this.cd(6500 - addTalent3*250,3)
					...
					this.blueComput(60 + addTalent3*6,'reduce')
					this.fasheArrow(420,2,39,this.myHero.id,3,2,1)
					if(this.enemyX < this.characterX){
						this.characterX = this.characterX + (80 + addTalent3*10)
					}else{
						this.characterX = this.characterX - (80 + addTalent3*10)
					}
				}
				if(num == 4){
					this.cd(15000 - addTalent4*300,4)
					this.playVoice('../../static/sounds/nj4.mp3',false,num-1,1500)
					setTimeout(()=>{
						this.playVoice('../../static/sounds/nj4_2.mp3',false,num-1,1500)
					},1500)
					this.delayTel(2500,num)
					this.blueComput(100 + addTalent4*10,'reduce')
					setTimeout(()=>{
						let val = this.harmComputer(1,addTalent4) + 350 + addTalent4*50 + this.randomNum(0,300)
						this.beat(val)
						let enfy = parseInt(this.enemy.fy*(0.3+addTalent4*0.07))
						this.enemy.fy = this.enemy.fy - enfy
						setTimeout(()=>{
							this.enemy.fy = this.enemy.fy + enfy
						},3000+addTalent4*700)
					},2700)
		...
		//所有技能延时
		delayTel(time,num) {
			if(this.$refs.tallents.tallentShow){
				this.$refs.tallents.tallentShow = false
				setTimeout(()=>{
					this.$refs.tallents.tallentShow = true
				},time)
			}
			if(this.$refs.tallents.tallentShow2){
				this.$refs.tallents.tallentShow2 = false
				setTimeout(()=>{
					this.$refs.tallents.tallentShow2 = true
				},time)
			}
			if(this.$refs.tallents.tallentShow3){
				this.$refs.tallents.tallentShow3 = false
				setTimeout(()=>{
					this.$refs.tallents.tallentShow3 = true
				},time)
			}
			if(this.$refs.tallents.tallentShow4){
		...
		regGj() {
			...
			this.playVoice('../../static/sounds/'+this.myHero.id+'.wav',true,7,1000)
			this.cd5 = false
			let v = parseFloat(this.myHero.v) * 1000
			let val = (parseInt(this.myHero.gj * 2.2) - parseInt(this.enemy.fy * 2.2)) * 1.5 + this.regGjAdd.gjHarm
			let hasBj = false
			//暴击
		...
		getEqlist(list,type) {
			...
			templist2.forEach(item=>{
				...
				if(item.name == '迅速进军') {
					this.antijiansu = this.antijiansu + 0.6
				}
				if(item.name == '纳什之牙') {
		...
		useTool(name) {
			...
			if(name == '法术提升药水') {
				this.myHero.fs = parseInt(this.myHero.fs) + 50
				setTimeout(()=>{
					this.myHero.fs = parseInt(this.myHero.fs) - 50
				},6000)
			}
			if(name == '攻击提升药水') {
				this.myHero.gj = parseInt(this.myHero.gj) + 35
				setTimeout(()=>{
					this.myHero.gj = parseInt(this.myHero.gj) - 35
				},6000)
			}
		...
		gameLoop() {
			if(this.myHero.id == 1006){	//地狱火被动
				this.myHero.fs += 15
				this.addBlood(200,0,0)
			}
			if(this.myHero.id == 1009){	//不朽战神被动
				this.myHero.gj = this.myHero.gj + 5 + this.reallyRandom(0,3)
				this.myHero.fy = this.myHero.fy + 5 + this.reallyRandom(0,3)
			}
			...
			if(this.gameType == 'shijiebei') {
				...
				//资格赛第二轮
				if(self.loop == 2) {
					self.addBlood(200,0,0)
					res3.data.concat(res4.data.slice(1)).forEach(item=>{
						if(!item.isOther && item.id != self.enemy.id){
							let ite = {...item}
							ite.gj = parseInt(ite.gj*1.6) + parseInt(ite.fs*0.35)
							ite.fy = parseInt(ite.fy*1.6)
							ite.blood = parseInt(ite.blood*1.3)
							ite.gs = ite.v
							ite.mk = ite.fy
					...
					text = '欢迎参加世界杯资格赛第二轮！'
					self.enemy.speed = 0.9
					self.enspeed = self.enemy.speed
					li = resp2.slice()
					let leng = li.length
					self.enemy = {...li[self.randomNum(0,leng - 1)]}
					self.enemyGJdis = self.enemy.distance
					self.blood1 = self.enemy.blood
				}
				//资格赛第三轮
				if(self.loop == 3) {
					self.addBlood(500,0,0)
					res3.data.concat(res4.data.slice(1)).forEach(item=>{
						if(!item.isOther && item.id != self.enemy.id){
							let ite = {...item}
							ite.gj = parseInt(ite.gj*1.65) + parseInt(ite.fs*0.35)
							ite.fy = parseInt(ite.fy*1.6)
							ite.blood = parseInt(ite.blood*1.3)
							ite.gs = ite.v
							ite.mk = ite.fy
				...
				if(self.loop >= 4) {
					res3.data.concat(res4.data.slice(1)).forEach(item=>{
						if(!item.isOther && item.id != self.enemy.id){
							let ite = {...item}
							ite.gj = parseInt(ite.gj*1.65 + parseInt(ite.fs*0.35) + self.loop*5)
							ite.fy = parseInt(ite.fy*1.6 + self.loop*5)
							ite.blood = parseInt(ite.blood*1.3) + this.randomNum(100 + self.loop*200,300 + self.loop*350)
							ite.gs = parseFloat(parseFloat(ite.v) - parseFloat(self.loop*0.03)).toFixed(1)
							ite.mk = ite.fy
							ite.icon = 'hero/' + ite.icon
							ite.isHero = true
							resp2.push(ite)
					...
					self.enemyGJdis = self.enemy.distance
					self.blood1 = self.enemy.blood
					self.enemy.speed = 0.85
					self.enspeed = self.enemy.speed
					if(self.loop == 4) {
						self.addBlood(800,0,0)
						self.money = editMoney(700)
						text = '恭喜你击败了资格赛所有选手，成功进入到正赛,奖励700元'
					}
					if(self.loop == 5) {
						self.money = editMoney(1000)
						self.addBlood(1000,0,0)
						this.flz = parseInt(this.flz) + 200
						if(this.flz > parseInt(this.myHero.flz)){
							this.flz = this.myHero.flz
						}
						this.blueVal = Math.round(parseInt(this.flz) / parseInt(this.myHero.flz) * 100)
						text = '恭喜你进入到32强，后续会遇到更强的对手，奖励1000元'
					}
					if(self.loop == 6) {
						self.money = editMoney(1500)
						self.addBlood(1200,0,0)
						this.flz = parseInt(this.flz) + 350
						if(this.flz > parseInt(this.myHero.flz)){
							this.flz = this.myHero.flz
						}
						this.blueVal = Math.round(parseInt(this.flz) / parseInt(this.myHero.flz) * 100)
						text = '厉害了！祝贺你闯入16强！奖励1500元'
					}
					if(self.loop == 7) {
						self.money = editMoney(2000)
						self.addBlood(1500,0,0)
						this.flz = parseInt(this.flz) + 200
						if(this.flz > parseInt(this.myHero.flz)){
							this.flz = this.myHero.flz
						}
						this.blueVal = Math.round(parseInt(this.flz) / parseInt(this.myHero.flz) * 100)
						text = '恭喜你闯入世界杯的八强！继续加油，拿出干劲！奖励2000元'
					}
					if(self.loop == 8) {
						self.money = editMoney(2000)
						self.addBlood(1500,0,0)
						this.flz = parseInt(this.flz) + 250
						if(this.flz > parseInt(this.myHero.flz)){
							this.flz = this.myHero.flz
						}
						this.blueVal = Math.round(parseInt(this.flz) / parseInt(this.myHero.flz) * 100)
						this.enemy.gj = parseInt(this.enemy.gj*1.15)
						this.enemy.blood = parseInt(this.enemy.blood*1.2)
						this.enemy.speed = 0.75
						this.enemyGJdis = parseInt(this.enemyGJdis * 1.1)
						this.enspeed = this.enemy.speed
						this.blood1 = this.enemy.blood
						text = '恭喜你闯入世界杯的四强！下面的敌人会非常厉害。继续加油，拿出干劲！奖励2000元'
					}
					if(self.loop == 9) {
						self.money = editMoney(2500)
						self.addBlood(1800,0,0)
						this.flz = parseInt(this.flz) + 300
						if(this.flz > parseInt(this.myHero.flz)){
							this.flz = this.myHero.flz
						}
						this.blueVal = Math.round(parseInt(this.flz) / parseInt(this.myHero.flz) * 100)
						this.enemy.gj = parseInt(this.enemy.gj*1.15)
						this.enemy.blood = parseInt(this.enemy.blood*1.2)
						this.enemy.fy = parseInt(this.enemy.fy*1.2)
						this.enemy.mk = parseInt(this.enemy.mk*1.2)
						this.enemyGJdis = parseInt(this.enemyGJdis * 1.1)
						this.enemy.speed = 0.7
						this.enspeed = this.enemy.speed
						this.blood1 = this.enemy.blood
						text = '恭喜你闯入世界杯的半决赛！调整好状态，迎接挑战吧！奖励2500元'
					}
					if(self.loop == 10) {
						self.money = editMoney(3000)
						self.addBlood(2000,0,0)
						this.flz = parseInt(this.flz) + 300
						if(this.flz > parseInt(this.myHero.flz)){
							this.flz = this.myHero.flz
						}
						this.blueVal = Math.round(parseInt(this.flz) / parseInt(this.myHero.flz) * 100)
						this.enemy.gj = parseInt(this.enemy.gj*1.2)
						this.enemy.blood = parseInt(this.enemy.blood*1.25)
						this.enemy.fy = parseInt(this.enemy.fy*1.3)
						this.enemy.mk = parseInt(this.enemy.mk*1.3)
						this.enemyGJdis = parseInt(this.enemyGJdis * 1.15)
						this.enemy.speed = 0.7
						this.enspeed = this.enemy.speed
						this.blood1 = this.enemy.blood
						text = '终于，你进入决赛！恭喜！！奖励3000元'
					}
					if(self.loop == 11) {
						self.money = editMoney(5000)
						text = '祝贺您夺得本届世界杯的冠军！奖励您5000元'
						this.showEnIcon = false
						this.win = true
						this.bettleEnd = true
						this.showWinLogo2 = true
		...
		winBettle() {
			...
			this.beatTime = null
			if(this.gameType == 'jinbiao' || this.gameType == 'tianti' || this.gameType == 'shijiebei'){
				this.gameLoop()
			}else{
				this.showEnIcon = false
			}
			...
			if(this.gameType.indexOf('tanxian') != -1){
				...
				setTimeout(()=>{
					uni.redirectTo({
						url: "/pages/white?item="+JSON.stringify(this.myHero)+'&map='+'"'+this.bgImg+'"'+'&tanxianBattle=true'
					})
				},200)
				// uni.redirectTo({
				// 	url: "/pages/play/tanxianLandScap?item="+JSON.stringify(this.myHero)+'&type=win'+'&map='+'"'+this.bgImg+'"'
				// })
			}
			if(this.gameType == 'paiwei'){
				let time = parseInt(this.min) * 60 + parseInt(this.second)
		...
		loseBettle() {
			...
			setTimeout(()=>{
				...
				if(this.gameType.indexOf('tanxian') != -1){
					this.myHero = {...this.origHero}
					uni.redirectTo({
						url: "/pages/play/tanxianLandScap?item="+JSON.stringify(this.myHero)+'&type=lose'
					})
				}
			},2400)
			let self = this
		...
		getAddTalent(option) {
			...
			// 齐天大圣被动
			if(this.myHero.name == '齐天大圣' || this.myHero.name == '狂暴孙悟空'){
		...
		harmComputer(type,addTalent) {
			...
			//攻击
			if(type == 1){
				val = (parseInt(this.myHero.gj * (2.8+addTalent*0.2)) - parseInt(this.enemy.fy*4.3*this.pojia))*2
			}
			//法术
			if(type == 2){
				val = (parseInt(this.myHero.fs * (2.8 + addTalent*0.2)) - parseInt(this.enemy.mk*4.3*this.pomo))*2
			}
			//敌方英雄
			if(type == 3){
				val = (parseInt(this.enemy.gj * 2.8) - parseInt(this.myHero.fy*3))*2 + 100
			}
			if(val < 0){
				val = parseInt(val/2)
			}
		...
		enemyTallent(num) {
			...
			if(this.enemy.name.indexOf('德玛西亚之力') != -1){
				...
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'zhifeng_emo')
				if(num == 1){
			...
			if(this.enemy.name.indexOf('GG Bond') != -1){
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'aiying')
				if(num == 1){
					if(this.distanceComp(this.enemy.dis1)){
			...
			if(this.enemy.name.indexOf('少林武僧') != -1){
				...
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'laotie')
				if(num == 1){
			...
			if(this.enemy.name.indexOf('无极剑圣') != -1){
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'zhibing_emo')
				if(num != 2){
			...
			if(this.enemy.name.indexOf('广寒仙子-嫦娥') != -1){
				...
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'xiaoxian')
				if(num == 1){
					if(this.distanceComp(this.enemy.dis1)){
						this.ygbj = true
			...
			if(this.enemy.name.indexOf('精英战士') != -1){
				...
				if(num == 4){
					if(this.distanceComp(this.enemy.dis4)){
						...
						this.backBgImg('jyzs2',5000,1)
					}
				}
			}
			if(this.enemy.name.indexOf('风神') != -1){
				this.playVoice('../../static/sounds/fs'+num+'.wav',true,num+3,5000)
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'maoxiaomei')
				if(num == 1){
					if(this.distanceComp(150)){
			...
			if(this.enemy.name.indexOf('诺克萨斯之手') != -1){
				...
				if(num == 4){
					if(this.distanceComp(this.enemy.dis4)){
						...
						let val = parseInt(this.enemy.gj * 2.5) + 500 + cval
						setTimeout(()=>{
							this.enemyBeat2(val)
						},1300)
					}
			...
			if(this.enemy.name.indexOf('探险家') != -1){
				...
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'aicheng')
				if(num == 1){
					this.fasheArrow(220,1,30,"探险家",1,1,5)
				}
				if(num == 2){
					this.fasheArrow(185,1,25,"探险家",2,4,5)
				}
				if(num == 3){
					if(this.distanceComp(this.enemy.dis3)){
				...
				if(num == 4){
					...
					this.backBgImg('ez2',2200)
					setTimeout(()=>{
						this.fasheArrow(2000,1,40,"探险家",4,6,2)
					},1500)
				}
			}
			...
			if(this.enemy.name.indexOf('玉玲珑') != -1){
				...
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'aiting')
				if(num == 1){
					if(this.distanceComp(this.enemy.dis1)){
			...
			if(this.enemy.name.indexOf('刀锋之影') != -1){
				...
				if(num == 4){
					...
					//非射手、法师，距离在100码外，打不到
					if(this.myHero.distance < 100 && !this.distanceComp(100)){
						this.delayTel(1500,0)
					}
					this.yinshen2 = true
					setTimeout(()=>{
			...
			if(this.enemy.name.indexOf('达摩') != -1){
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'zhifeng_emo')
				if(num == 1){
					this.fasheArrow(270,1,43,'达摩',1,5)
				}
				if(num == 2){
					if(this.randomNum(0,2) == 1 && this.bloodVal2 <= 35){
						this.enDuobi(70+this.randomNum(0,10))
						this.enemy.fy = this.enemy.fy + 50
						setTimeout(()=>{
							this.enemy.fy = this.enemy.fy - 50
						},3500)
				...
				if(num == 3){
					this.heroAnm('../../static/imgs/hero/damo3.gif?'+ new Date().getTime(),1)
					this.backBgImg('damo3',1000,1)
					let val = this.harmComputer(3)+ 350
					this.enemyBeat2(val)
					this.tallentExpert(3,2,3000)
				}
				if(num == 4){
					setTimeout(()=>{
						this.myNoMove = true
						this.delayTel(1000,0)
						this.fastsp = this.myspeed - 0.5
						if(this.characterX <= this.enemyX){
							this.characterX = this.characterX - 120
						}else{
							this.characterX = this.characterX + 120
						}
						if(this.characterY <= this.enemyY){
							if(Math.abs(this.characterY - this.enemyY)<60){
								this.characterY = this.characterY - parseInt(Math.abs(this.characterY - this.enemyY)*1.5) 
							}else{
								this.characterY = this.characterY - parseInt(Math.abs(this.characterY - this.enemyY)*0.6)
							}
						}else{
							if(Math.abs(this.characterY - this.enemyY)<60){
								this.characterY = this.characterY + parseInt(Math.abs(this.characterY - this.enemyY)*1.5)
						...
						let val1 = (parseInt(this.enemy.gj * 2.7) - parseInt(this.myHero.fy*3.5))*2
						if(val1 <= 0){
							val1 = -100
						}
						let val2 = val1 + 720
						this.enemyBeat2(val2)
						this.computeDistance()
					},500)
					setTimeout(()=>{
						this.fastsp = 0
			...
			if(this.enemy.name.indexOf('疾风灵鹰') != -1){
				this.playVoice('../../static/sounds/jfly'+num+'.wav',false,num+3,4000)
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'zhibing_emo')
				if(num == 1){
					this.heroAnm('../../static/imgs/hero/jfly1.gif?'+ new Date().getTime(),1)
					this.backBgImg('jfly1',1000,1)
					setTimeout(()=>{
						let val = this.harmComputer(3) + 300
						this.enemyBeat2(val)
						this.consistEnemyBeat(this.randomNum(10+parseInt(this.enemy.gj*0.5),30+parseInt(this.myHero.fy*0.8)),500,1500)
					},300)
				}
				if(num == 2){
					if(this.randomNum(0,2) == 1){
						this.heroAnm('../../static/imgs/hero/jfly2.gif?'+ new Date().getTime(),1)
						this.backBgImg('jfly2',5500,1)
						this.fastsp2 = 0.5
						this.delayTel(5500,0)
						setTimeout(()=>{
							this.fastsp2 = 0
						},5500)
						setTimeout(()=>{
							if(this.randomNum(0,1) == 1 && this.distanceComp(135)){
								let val = this.harmComputer(3) + 500
								this.enemyBeat2(val)
								this.enChongci(135,2)
							}
						},2000)
					}
				}
				if(num == 3){
					this.enChongci(135,0)
					let val = this.harmComputer(3)+ 350
					this.enemyBeat2(val)
				}
				if(num == 4){
					if(this.distanceComp(this.enemyGJdis) && this.randomNum(0,1) == 1 && !this.shotDown){
						this.tallentExpert(1,2,5500)
						this.delayTel(5500,0)
						this.shotDown = true
						this.heroAnm('../../static/imgs/hero/jfly4.gif?'+ new Date().getTime(),1)
						this.backBgImg('jfly4',1500,1)
						this.enemyMoveContrl(1)
						let t = setInterval(()=>{
							if(this.characterX >= this.enemyX){
								this.characterX = this.enemyX + 15
							}else{
								this.characterX = this.enemyX - 15
							}
							this.characterY = this.enemyY - 10
						},250)
						setTimeout(()=>{
							clearInterval(t)
							this.shotDown = false
							let val = this.harmComputer(3) + 500 + this.enemy.gj
							this.enemyBeat2(val)
			...
			if(this.enemy.name.indexOf('敖丙') != -1){
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'zhibing_emo')
				if(num == 1){
					this.heroAnm('../../static/imgs/hero/aobin1.gif?'+ new Date().getTime(),1)
					this.backBgImg('aobin1',4600,1)
					this.delayTel(2500,0)
					if(this.distanceComp(100)){
						this.tallentExpert(1,2,2500)
					}
				}
				if(num == 2){
					this.heroAnm('../../static/imgs/hero/aobin2.gif?'+ new Date().getTime(),1)
					this.backBgImg('aobin2',1900,1)
					this.fasheArrow(250,1,45,'敌方射击',2,16,2)
				}
				if(num == 3){
					this.enmapObjects.map((i,d)=>{
						if(i.img == 'shuiwall.png'){
					...
					this.enmapObjects.push({
						left: this.enemyX - 100,
						top: this.enemyY - 100,
						width: 430,
						height: 430,
						img: 'shuiwall.png'
					});
					this.enemy.gj = this.enemy.gj + 60
					this.enemy.fy = this.enemy.fy + 35
					setTimeout(()=>{
						this.enmapObjects.map((i,d)=>{
							if(i.img == 'shuiwall.png'){
				...
				if(num == 4){
					setTimeout(()=>{
						this.fasheArrow(3500,1,35,"无",3,14)
						if(Math.abs(this.enemyX - this.characterX) <= 625 && Math.abs(this.enemyY - this.characterY)<=35){
							let val = parseInt(this.enemy.gj*1.75) - parseInt(this.myHero.fy*1.3) + 35
							if(val<=0){
								val = 90
							}
							this.consistEnemyBeat(val,500,3000)
			...
			if(this.enemy.name.indexOf('托塔天王') != -1){
				...
				if(num == 2){
					if(this.randomNum(0,1) == 1 && this.distanceComp(550)){
						this.heroAnm('../../static/imgs/hero/tttw2.gif?'+ new Date().getTime(),1)
						this.backBgImg('tttw2',2500,1)
						if(this.time4){
							let val = this.harmComputer(3) + 100
							this.enemyBeat2(val)
						}else{
							this.myHero.v = parseFloat(this.myHero.v + 0.3).toFixed(2)
							this.tallentExpert(1,2,2000)
							this.time4 = setInterval(()=>{
								if(this.enemyX > this.characterX){
									this.characterX = this.characterX + 5
								}else{
									this.characterX = this.characterX - 5
								}
								if(this.enemyY > this.characterY){
									this.characterY = this.characterY + 5
								}else{
									this.characterY = this.characterY - 5
								}
							},100)
				...
				if(num == 3){
					if(this.randomNum(0,1) == 1){
						...
						this.enemy.gj = parseInt(this.enemy.gj * 1.3)
						setTimeout(()=>{
							this.enemy.gj = parseInt(this.enemy.gj / 1.3)
						},3000)
					}
				...
				if(num == 4){
					if(this.randomNum(0,2) == 1 && !this.time4 && this.distanceComp(450)){
						this.tallentExpert(3,2,1500)
						this.enmapObjects.push({
							left: this.characterX - 50,
							top: this.characterY - 180,
						...
						setTimeout(()=>{
							...
							this.delayTel(3000,0)
							this.tallentExpert(1,2,3000)
							let val = this.harmComputer(3) + 300 + parseInt(this.enemy.gj*1.8)
							this.enemyBeat2(val)
						},1500)
						setTimeout(()=>{
							this.time4 = null
			...
			if(this.enemy.name.indexOf('隐世侠客') != -1){
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'xiaogang')
				if(num == 1){
					this.fasheArrow(170,1,37,'敌方射击',1,2,2)
					this.playVoice('../../static/sounds/ysxk'+num+'.wav',true,num+3,1200)
				}
				if(num == 2){
					if(this.randomNum(0,1) == 1){
						this.heroAnm('../../static/imgs/hero/ysxk2.gif?'+ new Date().getTime(),1)
						this.backBgImg('ysxk2',4000,1)
						this.fastsp2 = 0.5
						setTimeout(()=>{
							this.fastsp2 = 0
						},4000)
				...
				if(num == 3){
					if(this.distanceComp(155)){
						this.playVoice('../../static/sounds/ysxk'+num+'.wav',true,num+3,1200)
						this.heroChongci(155)
						if(this.enemyX > this.characterX){
							this.enemyX = this.characterX - 50
						}else{
							this.enemyX = this.characterX + 50
						...
						let val = this.harmComputer(3) + 280 + parseInt(this.enemy.gj*0.5)
						if(this.randomNum(0,1) == 1){
							val = parseInt(val*1.3)
						}
						this.enemyBeat2(val)
				...
				if(num == 4){
					if(this.randomNum(0,2) == 1){
						this.fasheArrow(150,1,45,"敌方射击",4,18,5)
						this.playVoice('../../static/sounds/ysxk'+num+'.wav',true,num+3,2200)
						setTimeout(()=>{
							this.fasheArrow(150,1,45,"敌方射击",4,18,5)
						},500)
						setTimeout(()=>{
							this.fasheArrow(170,1,50,"敌方射击",4,18,5)
						},1000)
						setTimeout(()=>{
							this.fasheArrow(180,1,50,"敌方射击",4,18,5)
						},1500)
					}
				}
			}
			if(this.enemy.name.indexOf('阿宁') != -1){
				this.playVoice('../../static/sounds/aning'+num+'.wav',true,num+3,1200)
				this.playVoice(this.enemy.tallent[num].name,true,num+3,2000,'aiting')
				if(num == 1){
					this.fasheArrow(250,1,37,'敌方射击-高伤害',1,9)
				}
				if(num == 2){
					let val = parseInt(this.enemy.gj*1.5) - parseInt(this.myHero.fy*1.5) + 70 + this.randomNum(50,80)
					if(val<50){
						val = 50
					}
					this.consistEnemyBeat(val,500,2500)
					if(this.randomNum(0,2) == 2){
						//被动
						this.fasheArrow(220,1,37,'敌方射击',1,9)
					}
				}
				if(num == 3){
					if(this.randomNum(0,1) == 1){
						this.enChongci(100,1)
						if(this.distanceComp(90)){
							let val = this.harmComputer(3) + 300
							if(this.bloodVal2 <= 20){
								val = parseInt(val*2)
							}
							this.enemyBeat2(val)
						}
				...
				if(num == 4){
					if(this.bloodVal1 <= 25){
						if(this.randomNum(0,1) == 0){
							this.enemyConsistAddBlood(1200,900,1000)
							this.tallentExpert(3,1,1500)
						}
					}else{
						if(this.randomNum(0,2) == 0){
							this.enemyConsistAddBlood(600,900,1000)
						}
					}
				}
			}
		...
		regGjHandle() {
			...
			if(this.myHero.name == '女警' || this.myHero.id == 103) {
				this.xuli = this.xuli + 1
				if(this.xuli > 4){
					let val = parseInt(this.myHero.gj * 2)
					this.beat(val,1)
					this.xuli = 0
				}
			}
			if(this.myHero.name == '二郎真君-杨戬' || this.myHero.id == 1007) {
				if(this.reallyRandom(0,2) == 1){
					if(this.myHero.bj < 0.7){
						this.myHero.bj = this.myHero.bj + 0.2
					}
				}
			}
		...
		enMoveEffect() {
			...
			if(this.mapObjects.length != 0 && (this.myHero.name == '地狱火' || this.myHero.id == 1006)){
				this.mapObjects.map((it,idx)=>{
					if(this.mapObjects[idx].img == 'huozhu.png'){
						let x = Math.abs(it.left+5 - this.enemyX)
						let y = Math.abs(it.top+5 - this.enemyY)
						let dis = Math.sqrt(x*x+y*y)
						let addTalent = parseInt(this.addData.add9)
						if(dis <= 40){	//踩到
							let val = parseInt(this.myHero.fs * (0.6 + addTalent*0.07)) - parseInt(this.enemy.mk*1.7) + this.randomNum(addTalent*15,50 + addTalent*10)
							if(val <= 30){
								val = this.randomNum(30,50)
							}
							if(this.xuli == 1){
								val = parseInt(this.myHero.fs * (0.6 + addTalent*0.07))
							}
							this.beat(val,1)
						}
					}
			...
			if(this.mapObjects.length != 0 && (this.myHero.name == '女警' || this.myHero.id == 103)){
				this.mapObjects.map((it,idx)=>{
					if(this.mapObjects[idx].img == 'jiazi.jpeg'){
						let x = Math.abs(it.left+5 - this.enemyX)
						let y = Math.abs(it.top+5 - this.enemyY)
						let dis = Math.sqrt(x*x+y*y)
						let addTalent = parseInt(this.addData.add9)
						if(dis <= 37){	//踩到
							let val = this.myHero.gj
							this.beat(val,1)
							this.tallentExpert(1,1,2000+parseInt(this.addData.add9)*500)
						}
					}
				})
			}
		...
		enChongci(dis,type) {
			...
			if(type == 1){
				...
				if(this.distance <= dis+8){
					...
					this.fastsp2 = this.enspeed - 0.2
					if(this.enemyX > this.characterX){
						this.enemyX = this.characterX + 30
					}
					if(this.enemyY > this.characterY){
		...
		tallentExpert(type,index,time,val) {
			if(index == 1){	//敌方被施加
				...
				if(type == 4){	//麻痹
					if(this.enemy.icon != 'mabi.png'){
						this.tempEnemyIcon = this.enemy.icon
						this.enemy.icon = 'mabi.png'
					}
					this.enspeed = this.enspeed + 0.7
					setTimeout(()=>{
						this.enspeed = this.enspeed - 0.7
						if(this.tempEnemyIcon != this.enemy.icon && this.enemy.icon == 'mabi.png'){
							this.enemy.icon = this.tempEnemyIcon
						}
					},time)
					this.shotDown = true
				...
				if(type == 5){	//害怕
					if(this.enemy.icon != 'haipa.png'){
						this.tempEnemyIcon = this.enemy.icon
						this.enemy.icon = 'haipa.png'
					}
					this.enNoMove = true
					this.shotDown = true
					this.enspeed = this.enspeed + 0.8
					setTimeout(()=>{
						if(this.enspeed >= 1.2){
							this.enspeed = this.enspeed - 0.8
						}
						this.shotDown = false
						this.enNoMove = false
						if(this.tempEnemyIcon != this.enemy.icon && this.enemy.icon == 'haipa.png'){
							this.enemy.icon = this.tempEnemyIcon
						}
					},2500)
					this.enDuobi(30)
				...
				if(type == 6){	//狂暴
					if(this.enemy.icon != 'kuangbao.png'){
						this.tempEnemyIcon = this.enemy.icon
						this.enemy.icon = 'kuangbao.png'
					}
					this.fastsp2 = 0.3
					this.enemy.gj = this.enemy.gj + 80
					this.enemyMoveContrl(2)
					...
					setTimeout(()=>{
						this.fastsp2 = 0
						this.enemy.gj = this.enemy.gj - 80
						if(this.tempEnemyIcon != this.enemy.icon && this.enemy.icon == 'kuangbao.png'){
							this.enemy.icon = this.tempEnemyIcon
						}
					},time)
				}
				if(type == 7){	//烧伤-25%
					let val = Math.ceil(parseInt(this.blood1)/16)
					this.beat(val,1)
					setTimeout(()=>{
						this.beat(val,1)
					},1000)
				...
				if(type == 8){	//中毒-20%
					let val = Math.ceil(parseInt(this.blood1)/20)
					this.beat(val,1)
					setTimeout(()=>{
						this.beat(val,1)
					},1000)
				...
				if(type == 9){	//虚弱
					if(this.enemy.icon != 'xuruo.png'){
						this.tempEnemyIcon = this.enemy.icon
						this.enemy.icon = 'xuruo.png'
					}
					this.enemy.gs = parseFloat(this.enemy.gs + 0.3).toFixed(2)
					this.enspeed = this.enspeed + 0.3
					let gjdown = Math.ceil(this.enemy.gj/2)
					this.enemy.gj = this.enemy.gj - gjdown
					setTimeout(()=>{
						this.enemy.gj = this.enemy.gj + gjdown
						this.enspeed = this.enspeed - 0.3
						this.enemy.gs = parseFloat(parseFloat(this.enemy.gs) - 0.3).toFixed(2)
						if(this.tempEnemyIcon != this.enemy.icon && this.enemy.icon == 'xuruo.png'){
							this.enemy.icon = this.tempEnemyIcon
						}
					},time)
				}
				...
				if(type == 4){	//麻痹
					if(this.myHero.icon != 'mabi.png'){
						this.myHero.icon = 'mabi.png'
					}
				...
				if(type == 5){	//害怕
					if(this.myHero.icon != 'haipa.png'){
						this.myHero.icon = 'haipa.png'
					}
					this.delayTel(2500,0)
				...
				if(type == 6){	//狂暴
					if(this.myHero.icon != 'kuangbao.png'){
						this.myHero.icon = 'kuangbao.png'
					}
					this.fastsp = 0.37
					this.myHero.gj = this.myHero.gj + 80
					this.myHero.fs = this.myHero.fs + 100
					if(parseFloat(this.myHero.v)>0.6){
						this.myHero.v = parseFloat(this.myHero.v - 0.3).toFixed(2)
						setTimeout(()=>{
							this.myHero.v = parseFloat(parseFloat(this.myHero.v) + 0.3).toFixed(2)
				...
				if(type == 7){	//烧伤-25%
					...
					let val = Math.ceil(parseInt(this.blood2)/16) + this.myHero.fy*2
					this.enemyBeat2(val)
					setTimeout(()=>{
						this.enemyBeat2(val)
					},1000)
				...
				if(type == 8){	//中毒-20%
					...
					let val = Math.ceil(parseInt(this.blood2)/20) + this.myHero.fy*2
					this.enemyBeat2(val)
					setTimeout(()=>{
						this.enemyBeat2(val)
					},1000)
				...
				if(type == 9){	//虚弱
					if(this.myHero.icon != 'xuruo.png'){
						this.myHero.icon = 'xuruo.png'
					}
					this.myHero.v = parseFloat(this.myHero.v + 0.3).toFixed(2)
					this.myspeed = this.myspeed + 0.3
					let gjdown2 = Math.ceil(this.myHero.gj/2)
					let fsdown = Math.ceil(this.myHero.fs/2)
					this.myHero.gj = this.myHero.gj - gjdown2
					this.myHero.fs = this.myHero.fs - fsdown
					setTimeout(()=>{
						this.myHero.gj = this.myHero.gj + gjdown2
						this.myHero.fs = this.myHero.fs + fsdown
						this.myspeed = this.myspeed - 0.3
		...
		fasheArrow(distance,index,range,heroId,num,type,speed) {
			if(index == 1){		//敌方发射
				...
				if(type == 8){	//雪花箭
					arrowType =  'leftbinghuaTX.png'
					arrowType2 =  'rightbinghuaTX.png'
					this.arrowWid2 = 110
					this.arrowHei2 = 20
				}
				if(type == 9){	//飞镖
					arrowType =  'leftFeibiaoTX.png'
					arrowType2 =  'rightFeibiaoTX.png'
					this.arrowWid2 = 110
					this.arrowWid2 = 27
				}
				if(type == 10){	//激光波
		...
		shejiBeat(index,heroId,num) {
			...
			if(index == 1){
				...
				//百花仙子
				if(this.myHero.id == 1002 && this.myImmunity){
					return
				}
				//灵鹰
				...
				if(heroId == '达摩'){
					let val = this.harmComputer(3) + 20
					this.enemyBeat2(val)
					setTimeout(()=>{
						this.enChongci(2000,2)
						let val = this.harmComputer(3)+ this.enemy.gj + 250
						this.enemyBeat2(val)
					},300+this.randomNum(0,10)*70)
				}
				if(heroId == '敌方射击'){
				...
				//百花仙子
				if(this.enemy.id == 1002 && this.enImmunity){
					return
				}
				if(heroId == 10){
					if(num == 1){
				...
				if(heroId == 23){
					if(num == 1){
						let val = this.harmComputer(1,addTalent) + 270 + addTalent*70
						if(strongIndex == 1 || strongIndex == 2){
							val = val + this.myHero.gj
						}
						this.beat(val)
						if(this.distanceComp(130)){
							this.consistBeat(this.randomNum(50,150),600,1800)
						}
					}
				}
				if(heroId == 24){
					if(num == 3){
						let val = this.harmComputer(2,addTalent3) + this.myHero.fs - this.enemy.mk + 150 + addTalent3*50
						this.beat(val)
						this.tallentExpert(3,1,3500)
						this.myHero.gj = this.myHero.gj + 35 + addTalent3*5
						setTimeout(()=>{
							this.myHero.gj = this.myHero.gj - 35 - addTalent3*5
						},3500)
					}
				...
				if(heroId == 1000){
					if(num == 1){
						this.computeDistance()
						let val = this.harmComputer(1,addTalent) + 250 + this.myHero.gj + addTalent*30 + parseInt(this.distance * 0.8)
						this.beat(val)
						this.enemy.fy = this.enemy.fy - parseInt(this.enemy.fy*0.13)
					}
					if(num == 2){
						let val = this.harmComputer(1,addTalent2) + 250 + addTalent2*30
						this.beat(val)
						this.tallentExpert(3,1,3000)
					}
					if(num == 4){
						let val = this.harmComputer(1,addTalent4) + 350 + parseInt(this.myHero.gj*1.5) + this.randomNum(0,250)+ addTalent4*60
						this.beat(val)
					}
				}
				if(heroId == 1002){
					if(num == 1){
						let val = this.harmComputer(2,addTalent) + 250
						this.beat(val)
						this.tallentExpert(1,1,2000)
					}
					if(num == 2){
						let val = this.harmComputer(2,addTalent3)
						this.beat(val)
						this.tallentExpert(8,1)
					}
				}
				if(heroId == 1005){
					if(num == 1){
				...
				if(heroId == 1006){
					if(num == 1){
						let val = 0
						if(this.xuli==1){
							val = parseInt(this.myHero.fs * (2.5 + addTalent*0.2))*2
							this.beat(val,1)
						}else{
							 val = this.harmComputer(2,addTalent) + 150
							 this.beat(val)
						}
					}
					if(num == 4){
						this.tallentExpert(7,1)
					}
				}
				//二郎真君-杨戬
				if(heroId == 1007){
					if(num == 3){
						let val = this.harmComputer(2,addTalent3) + 250
						this.beat(val)
					}
				}
				//无量仙翁
				if(heroId == 1008){
					let val = this.harmComputer(2,addTalent3) + 150
					this.tallentExpert(1,1,1000+addTalent3*250)
					this.enemy.mk = this.enemy.mk - 5 - addTalent3
					this.beat(val)
				}
				//不朽战神
				if(heroId == 1009){
					let val = parseInt(this.myHero.gj*2.6) + 200 + addTalent4*150
					this.beat(val,1)
				}
				//女警
				if(heroId == 103){
					...
					if(num == 3){
						let val = parseInt(this.myHero.gj*1.5) - this.enemy.fy + 100
						this.beat(val)
						if(strongIndex == 1){
							this.tallentExpert(1,1,2500)
						}else{
							this.tallentExpert(3,1,3000+addTalent3*500,0.7)
						}
					}
				}
			}
		...
		myMoveEffect() {
			...
			//阿宁
			if(this.myHero.id == 23 && this.xuli == 1){
				if(parseInt(this.addData.strongIndex) == 2){
					this.shanxian(118+ parseInt(this.addData.add10)*6)
				}else{
					this.shanxian(90+ parseInt(this.addData.add10)*6)
				}
				this.computeDistance()
				if((parseInt(this.addData.strongIndex) == 2 && this.distanceComp(118)) || this.distanceComp(90)){
					let val = this.harmComputer(1,parseInt(this.addData.add10)) + 200 + parseInt(this.addData.add10)*60
					if(val < 50){
						val = 50
					}
					//敌方生命值小于20%
					if(parseInt(this.blood1)/parseInt(this.enemy.blood) <= 0.2){
						val = val * (2 + parseInt(this.addData.add10)*0.25)
					}
			...
			//狂暴奎木狼
			if(this.myHero.id == 1001 && this.distance > 300){
				this.addBlood(20,0,0)
			}
			//牛魔王
			if(this.myHero.id == 1003){
				this.addBlood(this.randomNum(4,5),0,0)
			}
		},
		//点击背景图释放技能
		handleTap(e) {
			...
			//地狱火
			if(this.myHero.id == 1006 && this.$refs.tallents.tallentShow2){
				this.$refs.tallents.tallentShow2 = false
				this.cd(5500 - addTalent2*200,2)
				this.blueComput(35+addTalent2*3,'reduce')
				this.mapObjects.push({
					left: pageX - 28,
					top: pageY - 31,
					width: 80,
			...
			//女警
			if(this.myHero.id == 103 && this.$refs.tallents.tallentShow2){
				this.$refs.tallents.tallentShow2 = false
				this.cd(11000 - addTalent2*250,2)
				this.blueComput(50+addTalent2*5,'reduce')
				this.mapObjects.push({
